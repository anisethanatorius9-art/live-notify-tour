                ...
                <div class="py-6 md:py-8">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            <p class="text-gray-600 mt-2 text-sm md:text-base">{{ __('Explore amazing services and experiences') }}</p>
                        </div>

                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4">
                            <div class="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 sm:py-2 w-full sm:w-auto">
                                <p class="text-sm text-blue-600 font-medium">{{ $totalBookings }} {{ __('Bookings') }}</p>
                            </div>
                            <div class="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3 sm:py-2 w-full sm:w-auto">
            ...
            <div class="py-6 md:py-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="bg-white rounded-lg border border-gray-200 p-3 hover:shadow transition-shadow">
                            <div class="h-32 md:h-40 rounded-md overflow-hidden bg-gray-100">
                                @php
                            ...
                            <div class="mt-2">
                                <div class="flex items-start justify-between gap-2">
                                    <div class="flex-1">
                                        <h3 class="text-sm font-semibold text-gray-900 line-clamp-2">{{ $service->name }}</h3>
                                    ...
                                    <span class="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full flex-shrink-0">{{ $service->category }}</span>
                                </div>

                                <p class="text-xs text-gray-600 mt-2 line-clamp-2">{{ $service->description }}</p>
                                    ...
                                    <div class="flex items-center justify-between mt-3 gap-2">
                                        ...
                                        <a href="{{ route('bookings.create', $service) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-xs md:text-sm whitespace-nowrap">{{ __('Book') }}</a>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 p-8 md:p-12 text-center">
                        <div class="text-gray-500">
                            <p class="text-base md:text-lg">{{ __('No services found') }}</p>
                            <p class="text-xs md:text-sm mt-2">{{ __('Try adjusting your filters') }}</p>
                        </div>
                    </div>
                    @endif
                </div>
                ...
                <div class="mt-8 md:mt-12">
                    <h2 class="text-xl md:text-2xl font-bold text-gray-900">{{ __('National Parks') }}</h2>
                    <p class="text-xs md:text-sm text-gray-600 mt-2">{{ __('Discover a larger selection of East Africa parks with real photos and local pricing.') }}</p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-5">
                        ...
                        <div class="w-full bg-white rounded-lg border border-gray-200 p-3 hover:shadow transition-shadow">
                            <div class="h-32 md:h-40 rounded-md overflow-hidden bg-gray-100">
                                ...
                                @else
                                <div class="flex items-center justify-center h-full text-gray-400 text-sm">{{ __('No image') }}</div>
                                @endif
                            </div>
                            ...
                            <div class="mt-2">
                                <div class="flex items-start justify-between gap-2">
                                    <div class="flex-1">
                                        <h3 class="text-sm font-semibold text-gray-900 line-clamp-2">{{ $park['name'] }}</h3>
                                        <p class="text-xs text-gray-500 mt-1 flex items-center gap-2">
                                            <img src="https://flagcdn.com/16x12/{{ strtolower($park['flag']) }}.png" alt="{{ $park['flag'] }}" class="inline-block" />
                                            {{ $park['location'] }}
                                        </p>
                                    </div>
                                    <span class="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full flex-shrink-0">{{ __('Park') }}</span>
                                </div>

                                <div class="mt-3 flex items-center justify-between gap-2">
                                    <div>
                                        <div class="text-sm md:text-base font-bold text-gray-900">{{ $park['currency'] }} {{ number_format($park['price']) }}</div>
                                        <div class="text-xs text-gray-500">{{ __('per person') }}</div>
                                    </div>
                                    <a href="{{ route('parks.show', $slug) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-xs md:text-sm whitespace-nowrap">{{ __('View') }}</a>
                                </div>
                            </div>
                        </div>
                        @endforeach
